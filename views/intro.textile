...
A Sammy application is built around "Routes":docs/routes. Routes define a link between a named state (usual in the hash or the url) and the action taken at that state (as a callback/function). Routes are defined using the common HTTP verbs within a @Sammy.Application@ container. 
